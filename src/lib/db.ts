...
const defaultSqlitePath = path.join(process.cwd(), 'car_revision.db');
const tmpSqlitePath = path.join(os.tmpdir(), 'car_revision.db');
const sqlitePath = process.env.SQLITE_DB_PATH
  || (process.env.VERCEL === '1' || process.env.VERCEL_ENV === 'production' ? tmpSqlitePath : defaultSqlitePath);
...
class SqliteAdapter implements DbAdapter {
  ...
  constructor() {
    ...
    const candidatePaths = [sqlitePath, tmpSqlitePath];
    let lastError: unknown;

    for (const dbPath of candidatePaths) {
      try {
        this.connection = new DatabaseCtor(dbPath, { timeout: 10000 });
        console.info(`Opened SQLite DB at ${dbPath}`);
        break;
      } catch (err) {
  ...
  private async initialize() {
    ...
    try {
      this.exec('ALTER TABLE car_profile ADD COLUMN engine TEXT');
    } catch {
      // Already exists
    }

    try {
      this.exec('ALTER TABLE car_profile ADD COLUMN is_active INTEGER NOT NULL DEFAULT 0');
    } catch {
      // Already exists
    }

    try {
      this.exec('ALTER TABLE revisions ADD COLUMN car_id INTEGER NOT NULL DEFAULT 1');
    } catch {
      // Already exists
    }

    const activeCar = this.connection.prepare('SELECT id FROM car_profile WHERE is_active = 1 ORDER BY id LIMIT 1').get() as { id: number } | undefined;
    const fallbackCarId = activeCar?.id ?? (this.connection.prepare('SELECT id FROM car_profile ORDER BY id LIMIT 1').get() as { id: number } | undefined)?.id ?? 1;

    this.connection.prepare('UPDATE car_profile SET is_active = CASE WHEN id = ? THEN 1 ELSE 0 END').run(fallbackCarId);
    this.connection.prepare('UPDATE revisions SET car_id = ? WHERE car_id IS NULL').run(fallbackCarId);

    const carCountResult = this.connection.prepare('SELECT COUNT(*) as count FROM car_profile').get() as { count: number };
    if (carCountResult.count === 0) {
      const createdCar = this.connection.prepare(`
...
function createDbAdapter(): DbAdapter {
  return usePostgres ? new PostgresAdapter() : new SqliteAdapter();
}

export async function getDb(): Promise<DbAdapter> {
  if (!adapterInstance) {
    adapterInstance = createDbAdapter();
  }
